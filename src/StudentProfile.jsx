function StudentProfile({ setPage, user }) {
  return (
    <div style={{ fontFamily: 'Arial', minHeight: '100vh', background: '#f0f4ff' }}>
      <nav style={{ background: '#1435c3', padding: '15px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <button onClick={() => setPage('home')} style={{ background: 'white', color: '#1435c3', border: 'none', padding: '8px 20px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>Ana səhifə</button>
        </div>
      </nav>
      ...
      <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '10px', padding: '30px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          ...
          <p style={{ color: '#888' }}>Hələ heç bir kurs almamısınız. <span onClick={() => setPage('home')} style={{ color: '#1435c3', cursor: 'pointer' }}>Kurslara baxın →</span></p>
        </div>
